...
namespace SwissTransport
{
    public class Transport : ITransport
    {
        ...
        public StationBoardRoot GetStationBoardbyDateTime(string station, string id, DateTime date, DateTime time)
        {
            var request = CreateWebRequest("http://transport.opendata.ch/v1/stationboard?Station=" + station + "&id=" + id + "&datetime=" + date.ToString("yyyy-MM-dd") + time.ToString("HH:mm"));
            var response = request.GetResponse();
            var responseStream = response.GetResponseStream();
